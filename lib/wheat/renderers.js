  ...
  function postProcess(headers, buffer, version, path, callback) {
    Step(
      function buildHeaders() {
        ...
        // always cache
        headers["ETag"] = MD5.md5(version + ":" + path);
  ...
  return {
    ...
    article: Git.safe(function renderArticle(version, name, callback) {
      ...
      Step(
        ...
        function applyTemplate(err, markdown, description, categories) {
          if (err) { callback(err); return; }
    ...
    staticFile: Git.safe(function staticFile(version, path, callback) {
      Step(
        function loadPublicFiles() {
          Git.readFile(version, "themes/" + config.theme + "/public/" + path, this);
        },
    ...
    fail: function(version, callback){
